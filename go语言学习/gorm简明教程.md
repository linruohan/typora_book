...
<img src="https://avatars.githubusercontent.com/u/30968856?v=4" alt="@Darkells" style="zoom:50%;" />

[JimmyLincole](https://github.com/JimmyLincole)发表于超过 2 年前
...
User 结构体的属性 Birthday 和 MemberNumber，使用指针，是有什么含义么？谢谢

默认所有字段的零值, 比如 0, '', false 或者其它 零值，都不会保存到数据库内，使==用指针可以避免这种情况==。

```
PS:不给值的说法是不严谨的，例如我给某字段赋予一个默认的零值，此时这个值并不会保存到数据库，而是使用默认值。
```

请教一个问题，gorm的时间格式化问题：

```
我的model中时间相关的结构体定义如下：
type ModelTime struct {
	CreatedAt time.Time  `json:"created_at"`
	UpdatedAt time.Time  `json:"updated_at"`
	DeletedAt *time.Time `sql:"index" json:"deleted_at"`
}
 一切正常，但是查询的时候，时间格式："created_at": "2021-02-04T19:58:11.1637748+08:00",
我想把它格式化成 Y-m-d H:i:s 这种格式
...
粘贴上述代码，查询的时候，时间果然正常了（Y-m-d H:i:s）
```

> 七米老师，我自己找到了一种解决方案，我在定义时间的时候，使用了gorm指定了一下时间格式
...
试验结果：
{
    ...
    "data": {
        ...
        "name": "搞笑类",
        "desc": "这是一个搞笑题材为主的博客",
        "created_at": "2021-02-04 20:30:21",
        "updated_at": "2021-02-04 20:30:21",
        "deleted_at": null
    },
    "msg": "success"
}
 迁移和查询都符合自己的要求了，谢谢七米老师
```
