# poetry
使用 poetry 作为项目的虚拟环境已经蛮久了，这个工具虽然非常好用，但是一致没有用于生成环境之中，也没有力荐这个工具的使用。 如果感觉差不多是时候了，所以打算把虚拟环境改为 `poetry`，不在使用其他的虚拟环境。

相比 pip 与 venv 的使用，poetry 确实有一定的学习门槛，需要先理清楚 pip 与 虚拟环境之间的关系，然后才能学习 poetry 的使用。 现在我把自己的项目全部用 poetry 来进行管理，使用体验远远大于我之前用过的 `pip + venv`、`pyenv`、`conda` 的虚拟环境。

## 1 poetry 是什么[​](https://notes.zhengxinonly.com/environment/use-poetry.html#poetry-%E6%98%AF%E4%BB%80%E4%B9%88 "​")

> [Poetry - Python dependency management and packaging made easy](https://python-poetry.org/ "Poetry - Python dependency management and packaging made easy")
...
#### 2.3.1 创建虚拟环境 
```bash
# 默认是在 C:\Users\xxp\AppData\Local\pypoetry\Cache\virtualenvs\poetry-demo-Ut74gzEx-py3.10 目录下
poetry env use python
# 修改为创建在当前目录下
poetry config virtualenvs.in-project true  # 设置在当前目录下.env
poetry env remove python  # 删除之前创建的虚拟环境
poetry env use python  # 重新创建
```
**重点说明**
- `poetry env use python` 是使用当前命令行下激活的 python 解释器创建虚拟环境
    ...
    - 更多的配置可以查看 [官方文档](https://python-poetry.org/docs/managing-environments/ "官方文档")
...
Creating virtualenv poetry-demo in X:\poetry-demo\.venvUsing virtualenv: X:\poetry-demo\.venv
```
...
Spawning shell within X:\poetry-demo\.venv (poetry-demo-py3.10) X:\poetry-demo>
```
...
### 5.1 安装模块[​](https://notes.zhengxinonly.com/environment/use-poetry.html#%E5%AE%89%E8%A3%85%E6%A8%A1%E5%9D%97 "​")
...
```cobol
(poetry-demo-py3.10) X:\poetry-demo>poetry add flaskUsing version ^2.3.2 for flask Updating dependenciesResolving dependencies... Writing lock file Package operations: 8 installs, 0 updates, 0 removals   • Installing colorama (0.4.6)  • Installing markupsafe (2.1.3)  • Installing blinker (1.6.2)  • Installing click (8.1.6)  • Installing itsdangerous (2.1.2)  • Installing jinja2 (3.1.2)  • Installing werkzeug (2.3.6)
```
...
```csharp
[tool.poetry.dependencies]python = "^3.10"flask = "^2.3.2"  # 新增部分
```
...
### 5.2 `poetry.lock` 与更新顺序[​](https://notes.zhengxinonly.com/environment/use-poetry.html#poetry-lock-%E4%B8%8E%E6%9B%B4%E6%96%B0%E9%A1%BA%E5%BA%8F "​")
...
```cobol
poetry-demo├── poetry.lock└── pyproject.toml 0 directories, 2 files
```

### 5.3 poetry lock ：更新 poetry.lock[​](https://notes.zhengxinonly.com/environment/use-poetry.html#poetry-lock-%E6%9B%B4%E6%96%B0-poetry-lock "​")
...
### 5.4 新增模块至 dev-dependencies[​](https://notes.zhengxinonly.com/environment/use-poetry.html#%E6%96%B0%E5%A2%9E%E6%A8%A1%E5%9D%97%E8%87%B3-dev-dependencies "​")
...
```cobol
[tool.poetry.dependencies]python = "^3.10"flask = "^2.3.2"  [tool.poetry.group.dev.dependencies]black = "^23.7.0"
```
...
### 5.5 Poetry 更新模块[​](https://notes.zhengxinonly.com/environment/use-poetry.html#poetry-%E6%9B%B4%E6%96%B0%E6%A8%A1%E5%9D%97 "​")
...
关于 `poetry update` 的其余参数，请参考[文件](https://python-poetry.org/docs/cli/#update "文件")。

还一件重要的事，那就是关于模块版本的升级限制规则，取决于你在 `pyproject.toml` 中的设定。

### 5.6 列出全部模块清单[​](https://notes.zhengxinonly.com/environment/use-poetry.html#%E5%88%97%E5%87%BA%E5%85%A8%E9%83%A8%E6%A8%A1%E5%9D%97%E6%B8%85%E5%8D%95 "​")
...
### 5.7 Poetry 移除模块[​](https://notes.zhengxinonly.com/environment/use-poetry.html#poetry-%E7%A7%BB%E9%99%A4%E6%A8%A1%E5%9D%97 "​")
...
<mark style="background-color: yellow；color: black">poetry remove 的依赖解析</mark>

```csharp
poetry remove flask
```

### 5.8 输出 Poetry 虚拟环境的 requirements.txt[​](https://notes.zhengxinonly.com/environment/use-poetry.html#%E8%BE%93%E5%87%BA-poetry-%E8%99%9A%E6%8B%9F%E7%8E%AF%E5%A2%83%E7%9A%84-requirements-txt "​")
...
```cobol
black @ file:///Users/kyo/Library/Caches/pypoetry/artifacts/11/4c/fc/cd6d885e9f5be135b161e365b11312cff5920d7574c8446833d7a9b1a3/black-22.3.0-cp38-cp38-macosx_10_9_x86_64.whlclick @ file:///Users/kyo/Library/Caches/pypoetry/artifacts/f0/23/09/b13d61d1fa8b3cd7c26f67505638d55002e7105849de4c4432c28e1c0d/click-8.1.2-py3-none-any.whlmypy-extensions @ file:///Users/kyo/Library/Caches/pypoetry/artifacts/b6/a0/b0/a5dc9acd6fd12aba308634f21bb7cf0571448f20848797d7ecb327aa12/mypy_extensions-0.4.3-py2.py3-none-any.whl...
```

这呈现好像不是我们以前熟悉的那样：

```cobol
black==22.3.0click==8.1.2...
```
...
### 5.9 输出 dev-dependencies[​](https://notes.zhengxinonly.com/environment/use-poetry.html#%E8%BE%93%E5%87%BA-dev-dependencies "​")
...
```cobol
black==22.3.0; python_full_version >= "3.6.2"click==8.1.2; python_version >= "3.7" and python_full_version >= "3.6.2"colorama==0.4.4; python_version >= "3.7" and python_full_version >= "3.6.2" and platform_system == "Windows"...
```
...
`poetry export` 所有参数用法与说明，请参考 [文件](https://python-poetry.org/docs/cli/#export "文件")。

## 6 Poetry 常用指令清单[​](https://notes.zhengxinonly.com/environment/use-poetry.html#poetry-%E5%B8%B8%E7%94%A8%E6%8C%87%E4%BB%A4%E6%B8%85%E5%8D%95 "​")
...
## 7 修改 poetry 镜像源[​](https://notes.zhengxinonly.com/environment/use-poetry.html#%E4%BF%AE%E6%94%B9-poetry-%E9%95%9C%E5%83%8F%E6%BA%90 "​")
